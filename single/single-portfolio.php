...
*/
get_header();
$portfolio_builder_elementor_related_post_meta = get_post_meta( get_queried_object_id(), 'swp_portfolio_meta' );
while ( have_posts() ) :   the_post();
$portfolio_builder_elementor_info = get_post_meta( get_the_id(), 'swp_portfolio_meta', true ); 

$swp_gallery_opt = $portfolio_builder_elementor_info['gallery']; // for eg. 15,50,70,125
        ...
        <div class="swp-row">
            ...
            <div class="swp-col-lg-4">
                <div class="swp-portfolio-single-sitebar">
                     <?php if( isset( $portfolio_builder_elementor_info[ 'info' ] ) && is_array( $portfolio_builder_elementor_info[ 'info' ] ) ): ?>
                        <ul>
                            <?php foreach( $portfolio_builder_elementor_info[ 'info' ] as $item ) : ?>
                                <li><strong><?php echo esc_html( $item[ 'type' ] ); ?></strong> <span><?php echo esc_html( $item[ 'details' ] ); ?></span></li>
                            <?php endforeach; ?>
                        </ul>
                    <?php endif; ?>

                    <?php if( isset( $portfolio_builder_elementor_info[ 'social' ] ) && is_array( $portfolio_builder_elementor_info[ 'social' ] ) ): ?>
                    <div class="swp-social-area">
                        <?php foreach( $portfolio_builder_elementor_info[ 'social' ] as $item ) : ?>
...
endwhile;
if( 1 == $portfolio_builder_elementor_related_post_meta[0][ 'related_post' ] ) :
...
<section class="portfolio-area style-1 swp-related">
    <div class="swp-container">
        <div class="swp-row">
            <div class="swp-col-12">
                <div class="style-number">
                    <h3><?php echo esc_html( $portfolio_builder_elementor_related_post_meta[0][ 'related_text' ] ); ?></h3>
                </div>
            </div>
        </div>
        <div class="swp-row">
        <?php
        //the query
        $portfolio_builder_elementor_related_post = new WP_Query( $args );

        if ( $portfolio_builder_elementor_related_post->have_posts() ):
            while ( $portfolio_builder_elementor_related_post->have_posts() ): $portfolio_builder_elementor_related_post->the_post();
            $swp_portfolio_meta = get_post_meta( get_the_id(), 'swp_portfolio_meta' );
